...
from lib import threestream,threestreamlstm,SlowFast
...
def train(model, train_dataloader, epoch, criterion, optimizer, writer):
    ...
    for step, (inputs, labels) in enumerate(train_dataloader):
        ...
        all_preds = torch.cat( (all_preds, outputs), dim=0).cuda()
        label_float = torch.tensor(labels, dtype=torch.float).cuda()
        lbl = torch.cat( (lbl, label_float), dim=0)
    ...
    pred_max = all_preds.argmax(dim = 1)
    cm = confusion_matrix(pred_max.cpu(), lbl.cpu())
    #print("-------------------------------------------------------------------------------------------------------")
    #print(cm.shape)
    np.savetxt("foo.csv", cm, delimiter=",")
